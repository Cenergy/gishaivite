...
const getUuidByName = (modelName: string) => {
  return modelLoader.getUuidByName(modelName, modelOptions.value)
}
    ...
    if (result.model) {
      ...
      if (result.performanceStats) {
        ...
        if (result.performanceStats.chunkSize !== undefined) {
          performanceStats['分块大小'] = modelLoader.formatBytes(result.performanceStats.chunkSize)
        }
        ...
        if (result.performanceStats.originalSize !== undefined) {
          performanceStats['原始大小'] = modelLoader.formatBytes(result.performanceStats.originalSize)
        }
        if (result.performanceStats.compressedSize !== undefined) {
          performanceStats['压缩大小'] = modelLoader.formatBytes(result.performanceStats.compressedSize)
        }
        if (result.performanceStats.averageSpeed !== undefined) {
          performanceStats['平均速度'] = modelLoader.formatBytes(result.performanceStats.averageSpeed) + '/s'
        }
